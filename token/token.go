...
type EasyToken struct {
	...
	Expires int64
	//  En este caso se usara para el rol de usuario
	Role string
}
...
const (
	privKeyPath = "token/rsakey.pem"     // openssl genrsa -out app.rsa keysize
	pubKeyPath  = "token/rsakey.pem.pub" // openssl rsa -in app.rsa -pubout > app.rsa.pub
)
...
func (e EasyToken) GetToken() (string, error) {
	// Create the Claims
	claims := &jwt.StandardClaims{ // a partir de la normativa https://tools.ietf.org/html/rfc7519#section-4.1
...
// funcion que valida el rol de un token para permitir x acción
func (e EasyToken) ValidateRoleToken(tokenString string, role int) (bool, string, error) {
	if tokenString == "" {
		return false, "", errors.New("token is empty")
	}
	...
	if token.Valid {
		...
		fmt.Println("Token:")
		roleToken, err := strconv.Atoi(claims["sub"].(string))
		if err != nil {
			return false, "", err
		} else {

			if roleToken <= role {
				fmt.Println(roleToken, true)
				return true, iss, nil
			} else {
